...
class Server(object):
    def __init__(self, args, times):
        ...
        self.uploaded_weightsuploaded_weights = []
        self.uploaded_ids = []
    ...
    def headavg_old(self, args, local_nets_dict, head_weights_keys, user_cat_index=None):
        if user_cat_index is None:
            user_cat_index = self.generate_default_user_cat_index(args.num_users,
    ...
    def headavg(self, args, local_nets_dict, head_weights_keys):
        dict_length = args.num_classes - args.novel_class_num
        cate_weight_component_dict = dict(zip([i for i in range(dict_length)], [[] for _ in range(dict_length)]))

        for idx in range(len(local_nets_dict)):
            for key in head_weights_keys:
                if 'bias' not in key:
                    for line in range(dict_length):
                        if args.cos:
                            cate_weight_component_dict[line].append(
                                copy.deepcopy(local_nets_dict[idx][key])[:, line].expand(1, -1))
                        else:
    ...
    def generate_default_user_cat_index(self, num_users, num_classes):
        user_cat_index = [list(range(num_classes)) for _ in range(num_users)]
        return user_cat_index